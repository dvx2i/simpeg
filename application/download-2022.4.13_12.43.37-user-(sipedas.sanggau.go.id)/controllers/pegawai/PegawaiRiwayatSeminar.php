...
/**
 * Description of PegawaiRiwayatSeminar
 *
 * @author Zanuar
 */
class PegawaiRiwayatSeminar extends MY_Controller
{
    ...
    function view($nip)
    {
        $data['result'] = $this->m_pegawai_diklat->get_where(array('diklat_jenis' => 'SEMINAR', 'diklat_pegawai_nip' => $nip));
        $data['pegawai'] = $this->m_pegawai->get_row($nip);
        if ($this->cek_admin_opd($data['pegawai'])) {
            $this->loadView('pegawai/pegawai_riwayat_seminar', $data);
        } else {
    ...
    function add()
    {
        ...
        if (!empty($insert)) {
            $this->session->set_flashdata('message', alert_show('success', "Tambah Riwayat Seminar Pegawai Berhasil"));
        } else {
            $this->session->set_flashdata('message', alert_show('danger', "Tambah Riwayat Seminar Pegawai Gagal"));
        }
        redirect('pegawai/PegawaiRiwayatSeminar/view/' . $nip  . '#riwayat');
    }
    ...
    function update()
    {
        ...
        if (!empty($insert)) {
            $this->session->set_flashdata('message', alert_show('success', "Update Riwayat Seminar Pegawai Berhasil"));
        } else {
            $this->session->set_flashdata('message', alert_show('danger', "Update Riwayat Seminar Pegawai Gagal"));
        }
        redirect('pegawai/PegawaiRiwayatSeminar/view/' . $nip . '#riwayat');
    }
    ...
    function delete($id)
    {
        ...
        if (!empty($delete)) {
            $this->session->set_flashdata('message', alert_show('success', "Hapus Riwayat Seminar Pegawai Berhasil"));
        } else {
            $this->session->set_flashdata('message', alert_show('danger', "Hapus Riwayat Seminar Pegawai Gagal"));
        }
        redirect('pegawai/PegawaiRiwayatSeminar/view/' . $nip . '#riwayat');
        echo $nip;
    }
