...
ORDER BY pegawaipangkat_pangkat_id DESC LIMIT 1;";
    ...
    {
        $query= "SELECT pegawaikgb_tmt,pegawai_nip,d.pegawaikgb_pangkat_id,pegawai_pangkat_terakhir_id,a.`pegawai_gaji`,c.`gaji_jumlah`,c.`gaji_masa_kerja`,d.pegawaikgb_masa_kerja_bulan,pangkat_golongan_text FROM pegawai a
  ...
  INNER JOIN (SELECT MAX(pegawaikgb_tmt) AS pegawaikgb_tmt_old,pegawaikgb_pegawai_nip
        ...
        LEFT JOIN ref_gaji c ON b.`pangkat_golongan_kode` = c.`gaji_golru_kode` AND c.`gaji_masa_kerja` = (`d`.`pegawaikgb_masa_kerja_tahun`+2)
...
//     }
  ...
  INNER JOIN (SELECT MAX(pegawaikgb_tmt) AS pegawaikgb_tmt_old,pegawaikgb_pegawai_nip
    ...
    $this->datatables->add_column('aksi', '$1', 'aksi_kgb(pegawaikgb_id,"'.$this->input->post('proses').'",pegawai_nip,pegawai_nama)');
