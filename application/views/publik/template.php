            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="header-navbar">
                    <ul class="nav navbar-nav navbar-right">
                        <li class="nav-item">
                            <a class="nav-link <?= $this->uri->segment(1) == 'Home' && empty($this->uri->segment(2)) ? 'active' : ''; ?>" href="<?= site_url('Home') ?>">HOME</a>
                        </li>
                        <li class="nav-item"><a class="nav-link" href="#struktur">STRUKTUR ORGANISASI</a></li>
                        <li class="nav-item"><a class="nav-link <?= $this->uri->segment(1) == 'cuti' ? 'active' : ''; ?>" href="<?= site_url('cuti_online') ?>">CUTI ONLINE</a></li>
                        
                        <?php if ($this->uri->segment(1) <> 'cuti') : ?>
<!--                     	<li class="nav-item"><a class="nav-link" href="<?= site_url('Akun/register') ?>">MUTASI ONLINE</a></li> -->
                        <?php endif ?>
                    	<li class="nav-item"><a class="nav-link <?= $this->uri->segment(2) == 'kontak' ? 'active' : ''; ?>" href="<?= site_url('Home/kontak') ?>">KONTAK</a></li>
                    
                        <?php if (empty($this->session->userdata('login'))) : ?>
                            <li class="nav-item"><a class="nav-link" href="<?= site_url('Akun') ?>">LOGIN</a></li>
                        <?php endif ?>
                        <?php if (!empty($this->session->userdata('login') && $this->session->userdata('login')['group_id'] <> "6" && $this->uri->segment(1) <> 'cuti' )) : ?>
                            <li class="nav-item"><a class="nav-link" href="<?= site_url() ?>">ADMIN</a></li>
                        <?php endif ?>
                        <?php if (!empty($this->session->userdata('login') && $this->session->userdata('login')['group_id'] == "6" )) : ?>
                            <li class="nav-item"><a class="nav-link" href="<?= site_url('Akun/logout') ?>">LOGOUT</a></li>
                        <?php endif ?>
                    </ul>
                </div>
                <!-- end navbar-collapse -->
        ...
        <div id="footer" class="footer">
			<div class="container">
				<div class="footer-brand">
                    <img class="footer-img" src="<?= base_url('assets/publik') ?>/img/logo-2021.png" alt="">
                    <img class="bsre-img" src="<?= base_url('assets/publik') ?>/img/bsre-logo.png" alt="">
				</div>
				<p>
                    <strong>Copyright &copy; 2020 SIPEDAS.</strong> 
					Sistem Informasi Pelayanan Data ASN
				</p>
			</div>
		</div>
        <!-- end #footer -->
