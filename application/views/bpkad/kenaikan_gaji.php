                                        ...
                                        <div class="color-palette-set">
                                            ...
                                            <div class="color-palette">
                                                <select name="proses" class="form-control select2" id="proses">
                                                    <option <?= $filter['proses'] == '4' ? 'selected' : ''; ?> value="4">Belum Diproses</option>
                                                    <option <?= $filter['proses'] == '5' ? 'selected' : ''; ?>  value="5">Sudah Diproses</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">File SK Kenaikan Gaji Berkala</h5>
...
<script>
    ...
    function proses() {
        ...
        swal({
            ...
            if (isConfirm) {
                location.href = '<?= site_url('bpkad/Kenaikan_gaji/update') ?>'
            } else {
    ...
    $(document).ready(function() {
        ...
        tableData = $('#tabelss').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                url: '<?= site_url('bpkad/Kenaikan_gaji/json') ?>',
                type: 'POST',
                data: function(data) {
        ...
        $('#verifBtn').on('click', function() {
            ...
            // }, function(isConfirm) {
                // if (isConfirm) {
                    var cpns = $('#cpns').val();
                    // var form = "<form id='hidden-form' action='<?= site_url('bpkad/Kenaikan_gaji/update') ?>' method='post' target='_blank'>";
                    ...
                    $.ajax({
                        url: '<?= site_url('bpkad/Kenaikan_gaji/update') ?>',
                        type: "POST",
                        data: {
                            ajax: '1',
                            list_id: list_id
    ...
    function d_m_y(date) {
		...
		$('#btn-upload').click(function(){
        	...
        	var formData = new FormData(frm[0]);
							...
							$.ajax({
								...
								url: "<?= site_url('bpkad/Kenaikan_gaji/update/sk/') ?>",
								dataType: "JSON",
								data: formData,
								processData: false,
								contentType: false,
        ...
        $(document).on('click', '.btn-sk', function(){
            ...
            success: function(data) {
                ...
                console.log(data)
                var html = '<a href="<?= base_url("assets/docs/kgb/") ?>' + data[0].kgbsk_file + '" class="btn btn-sm btn-default" target="_blank"><i class="fa fa-download"></i> <small>Unduh</small></a>';
                ...
                var berkasModal = '<object data="<?= base_url("assets/docs/kgb/") ?>/' + data[0].kgbsk_file + '" type="application/pdf" width="100%" height="480px"></object>';
                $("#berkasModal" ).find(".berkasModal").append(berkasModal);
                var footerModal = '<a href="<?= base_url("assets/files/") ?>/' + data[0].kgbsk_file + '" class="btn btn-success" target="_blank"> Unduh</a><button type="button" class="btn btn-primary" data-dismiss="modal">Kembali</button>'
