...
class Kenaikan_pangkat extends MY_Controller {
    ...
    //variable
    var $view = 'bpkad/kenaikan_pangkat';     // file view
    var $redirect = 'bpkad/Kenaikan_pangkat';     // redirect to here
    var $modul = 'Kenaikan Gaji Berkala';        // this modul or class name

    public function __construct() {
        parent::__construct();
        $this->load->model(array('m_pegawai_pangkat_bpkad', 'ref_pejabat'));
    ...
    public function json()
    {
        ...
        $this->session->set_userdata('filter', $filter);
            ...
            echo $this->m_pegawai_pangkat_bpkad->json();
    }
    ...
    private function upload_sk($nip){
        ...
        if ($_FILES["file"]["error"] == 0) {
            ...
            if ($this->upload->do_upload($fieldname)) {
                ...
                $this->m_pegawai_pangkat_bpkad->delete_file($id); //delete file yg sdh ada
                ...
                $this->m_pegawai_pangkat_bpkad->save_file($data_file);
    ...
    function update($jenis = null)
    {
        ...
        if (is_array($list_id)) {
            ...
            foreach ($list_id as $id) {
                ...
                $this->m_pegawai_pangkat_bpkad->update($data, $id);
                // $nomor  = $key->no_telp;
    ...
    public function cetak($kgb_id) {
        ...
        $PHPWord = new PHPWord();
		$row = $this->m_pegawai_pangkat_bpkad->get_by_id($kgb_id);

        if($row->pegawai_status_kepegawaian_id == '1'){ // jika cpns
            $kgb = $this->m_pegawai_pangkat_bpkad->get_data_cetak_cpns($kgb_id, $row->pegawaikgb_pegawai_nip);
        } else {
            $kgb = $this->m_pegawai_pangkat_bpkad->get_data_cetak($kgb_id, $row->pegawaikgb_pegawai_nip); 
        }
