...
ORDER BY pegawaipangkat_pangkat_id DESC LIMIT 1;";
  ...
  INNER JOIN (SELECT MAX(pegawaikgb_tmt) AS pegawaikgb_tmt_old,pegawaikgb_pegawai_nip
    ...
    {
        $query= "SELECT pegawai_cpns_tmt AS pegawaikgb_tmt,pegawai_nip,COALESCE(a.pegawai_hp,a.pegawai_telpon) AS no_telp,pegawai_pangkat_terakhir_id AS pegawaikgb_pangkat_id,pegawai_pangkat_terakhir_id,a.`pegawai_gaji`,c.`gaji_jumlah`,c.`gaji_masa_kerja`, '0' AS pegawaikgb_masa_kerja_bulan,pangkat_golongan_text FROM pegawai a
        LEFT JOIN ref_pangkat_golongan b ON a.`pegawai_pangkat_terakhir_id` = b.`pangkat_golongan_id`
...
//     }
  ...
  INNER JOIN (SELECT MAX(pegawaikgb_tmt) AS pegawaikgb_tmt_old,pegawaikgb_pegawai_nip
    ...
    $this->datatables->add_column('aksi', '$1', 'aksi_kgb(pegawaikgb_id,"'.$this->input->post('proses').'",pegawai_nip,pegawai_nama, proses_bpkad,kgbsk_file)');
