 ...
 * @author Zanuar
 */
class Pendanaan extends MY_Controller {

    var $page = "referensi/Pendanaan";

    public function __construct() {
        parent::__construct();
        $this->load->model('ref_pendanaan');
    }

    //put your code here

    public function index() {
        $data['result'] = $this->ref_pendanaan->get_all();
        $this->loadView('referensi/pendanaan', $data);
    }

    public function update() {
        $id = $this->input->post('id');
        $data['pendanaan_nama'] = $this->input->post('nama');
        $update = $this->ref_pendanaan->update($data, $id);
        if (!empty($update)) {
    ...
    public function add() {
        $data['pendanaan_nama'] = $this->input->post('nama');
        $simpan = $this->ref_pendanaan->insert($data);
        if (!empty($simpan)) {
    ...
    public function delete($id) {
        $simpan = $this->ref_pendanaan->delete($id);
        if (!empty($simpan)) {
