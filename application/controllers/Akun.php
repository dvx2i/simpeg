...
class Akun extends CI_Controller
{
    ...
    private function insert($pegawai)
    {
            $data['user_name'] = $pegawai->pegawai_nip;
            $data['user_pegawai_nip'] = $pegawai->pegawai_nip;
            $data['user_nama_lengkap'] = $pegawai->pegawai_nama;
            $data['user_password'] = sha1(1234);
            $data['user_unit_id'] = $pegawai->pegawai_unit_id;
            //        $data['UserIsActive'] = $this->input->post('status');
            $insert = $this->sys_user->insert($data);
            $group = '6';
            $id_user = $this->db->insert_id();
    ...
    function auth()
    {
        if ($_POST) {
            ...
            if ($auth != NULL) {
            	...
            	$pegawai = $this->m_pegawai->get_where(array('pegawai_nip' => $auth[0]['user_pegawai_nip'], 'pegawai_status' => '1'))->result();
            
            	if(empty($pegawai) && $username != 'admin' && $auth[0]['UserGroupGroupId'] != '11'){ // pegawai pensiun
                	redirect(site_url('Akun'));
            	}
                ...
                $data = array(
                    ...
                    'group_name' => $auth[0]['GroupName'],
                    'unit_id'  => $auth[0]['user_unit_id'],
                    'nip' => $auth[0]['user_pegawai_nip'],
                    'nik' => $auth[0]['pegawai_no_ktp'],
                    'foto'=> base_url()."/assets/images/".$auth[0]['pegawai_foto_kpe'],
                );
            ...
            $user = $this->m_user->get_where(array("user_name" => $username))->result_array();
            	
            if(empty($user)){
            	$nip = $username;
            	$pegawai = $this->m_pegawai->get_where(array('pegawai_nip' => $nip, 'pegawai_status' => '1'))->row();
            if(!empty($pegawai)){
            	$this->insert($pegawai);
            	
            $menu = $this->input->post('menu');
            $username = $pegawai->pegawai_nip;
            $password = $this->encrypt->sha1(1234);
            	$auth = $this->m_user->get_user_login("user_name = '$username' and user_password= '$password'")->result_array();
