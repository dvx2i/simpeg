...
class Pengajuan extends MY_Controller
{
    ...
    function create_action()
    {
    	...
    	$this->db->trans_start(); 
        $sk = $this->get_no_sk($this->input->post('jeniscuti_id'));
        $data['pegawaicuti_sk_no'] = $sk;
        ...
        if (!empty($insert_id)) {
            ...
            $berkas = $this->ref_berkas_cuti->get_all();
                ...
                if($item['berkas_id'] != '4' || $data['pegawaicuti_jeniscuti_id'] == '3' ) {
                    $id = $item['berkas_id'];
