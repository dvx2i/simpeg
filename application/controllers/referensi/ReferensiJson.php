...
class ReferensiJson extends CI_Controller
{
    ...
    public function getJumlahGajiByGolruMasaKerja()
    {
        $this->load->model('ref_gaji');
        $pangkat = $this->input->post('pangkat');
        $masa = $this->input->post('masa') != '' ? $this->input->post('masa') : 0;
        $output = $this->ref_gaji->getGajiByGolruMasaKerja($pangkat, $masa);
        // jsonResponse($output['gaji_jumlah']);
        echo $output['gaji_jumlah'];
    }
    ...
    public function getMutasiBerkas()
    {
        ...
        $data['usulan_id'] = $this->input->post('usulan_id');
        $data['berkas_mutasi'] = $this->m_pengajuan_mutasi->getBerkasByPermohonan($data['usulan_id']);
        $data['berkas'] =  $this->ref_berkas_mutasi->get_where(array('berkas_jenis_mutasi_id' => $jenis));
        $this->load->view('mutasi/pengajuan_berkas', $data);
    }
}
