...
class Jurusan extends MY_Controller {
    ...
    var $page = "referensi/Jurusan";

    public function __construct() {
        parent::__construct();
        $this->load->model('ref_jurusan');
    }

    //put your code here

    public function index() {
        $data['result'] = $this->ref_jurusan->get_all();
        $this->loadView('referensi/jurusan', $data);
    }

    public function update() {
        $id = $this->input->post('id');
        $data['jurusan_nama'] = $this->input->post('nama');
        $update = $this->ref_jurusan->update($data, $id);
        if (!empty($update)) {
    ...
    public function add() {
        $data['jurusan_nama'] = $this->input->post('nama');
        $simpan = $this->ref_jurusan->insert($data);
        if (!empty($simpan)) {
    ...
    public function delete($id) {
        $simpan = $this->ref_jurusan->delete($id);
        if (!empty($simpan)) {
