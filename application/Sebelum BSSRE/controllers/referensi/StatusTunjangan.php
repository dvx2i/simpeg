 ...
 * @author Zanuar
 */
class StatusTunjangan extends MY_Controller {

    var $page = "referensi/StatusTunjangan";

    public function __construct() {
        parent::__construct();
        $this->load->model('ref_status_tunjangan');
    }

    //put your code here

    public function index() {
        $data['result'] = $this->ref_status_tunjangan->get_all();
        $this->loadView('referensi/status_tunjangan', $data);
    }

    public function update() {
        $id = $this->input->post('id');
        $data['status_tunjangan_nama'] = $this->input->post('nama');
        $update = $this->ref_status_tunjangan->update($data, $id);
        if (!empty($update)) {
    ...
    public function add() {
        $data['status_tunjangan_nama'] = $this->input->post('nama');
        $simpan = $this->ref_status_tunjangan->insert($data);
        if (!empty($simpan)) {
    ...
    public function delete($id) {
        $simpan = $this->ref_status_tunjangan->delete($id);
        if (!empty($simpan)) {
