 ...
 * @author Zanuar
 */
class M_pegawai_cuti_online extends MY_Model
{
    ...
    function json()
    {
      ...
      $this->datatables->select('pegawaicuti_id,pegawaicuti_pegawai_nip,pangkat_golongan_text as pangkat,pegawaicuti_no_permohonan,pegawai_nama,CONCAT(IFNULL(pegawai_gelar_depan,"")," ",pegawai_nama," ",IFNULL(pegawai_gelar_belakang,"")) as nama,jenis_cuti_nama,pegawaicuti_status_permohonan,pegawaicuti_lama_cuti_mulai,pegawaicuti_lama_cuti_selesai,pegawaicuti_keterangan_tolak,file_sk,pegawaicuti_sk_no,pegawaicuti_sk_tanggal', FALSE);
      ...
      $this->datatables->join('ref_pangkat_golongan D', 'B.pegawai_pangkat_terakhir_id = D.pangkat_golongan_id', 'LEFT');
    ...
    //   $this->datatables->edit_column('file_sk', '$1', 'get_sk_mutasi(file_sk,usulan_status,"admin",usulan_id,insert_time)');
      ...
      $this->datatables->add_column('action', '$1', 'get_action_cuti(pegawaicuti_id,pegawaicuti_status_permohonan,pegawaicuti_sk_no,pegawaicuti_sk_tanggal,pegawaicuti_no_permohonan,pegawai_nama)');
    ...
    function get_by_no($id)
    {
        ...
        $this->db->select('A.pegawaicuti_id,A.pegawaicuti_no_permohonan,A.pegawaicuti_sk_no,A.pegawaicuti_sk_tanggal,A.pegawaicuti_pegawai_nip,A.pegawaicuti_jeniscuti_id,A.pegawaicuti_tahun,A.pegawaicuti_jumlah_hari,
        A.pegawaicuti_keterangan,A.pegawaicuti_status_permohonan,A.pegawaicuti_keterangan_tolak,A.pegawaicuti_bertahap,A.pegawaicuti_tahap,A.pegawaicuti_parent,A.file_sk,pegawai_nama,pegawai_unit_nama,pegawai_pangkat_terakhir_nama,pegawai_pangkat_terakhir_golru,pegawai_jabatan_nama,jenis_cuti_nama,A.pegawaicuti_lama_cuti_mulai,A.pegawaicuti_lama_cuti_selesai,B.pegawaicuti_lama_cuti_mulai as th2_mulai,B.pegawaicuti_lama_cuti_selesai as th2_selesai,B.pegawaicuti_jumlah_hari as th2_jumlah_hari,B.pegawaicuti_keterangan as th2_keterangan,status_nama', FALSE);
        $this->db->from('pegawai_cuti_online A');
        $this->db->join('pegawai', 'A.pegawaicuti_pegawai_nip = pegawai.pegawai_nip');
        $this->db->join('pegawai_cuti_online B', 'A.pegawaicuti_id = B.pegawaicuti_parent', 'LEFT');
    ...
    function get_data_cetak($id, $nip)
    {
        $this->db->select('A.*,P.*,U.unit_kpok,jenis_cuti_nama,B.pegawaicuti_lama_cuti_mulai as th2_mulai,B.pegawaicuti_lama_cuti_selesai as th2_selesai,B.pegawaicuti_jumlah_hari as th2_jumlah_hari,B.pegawaicuti_keterangan as th2_keterangan,status_nama,pangkat_golongan_nama,pangkat_golongan_pangkat', FALSE);
        $this->db->from('pegawai_cuti_online A');
        $this->db->join('pegawai P', 'A.pegawaicuti_pegawai_nip = P.pegawai_nip');
        $this->db->join('pegawai_cuti_online B', 'A.pegawaicuti_id = B.pegawaicuti_parent', 'LEFT');
        $this->db->join('ref_jenis_cuti', 'A.pegawaicuti_jeniscuti_id = ref_jenis_cuti.jenis_cuti_id');
        $this->db->join('ref_status_permohonan_cuti', 'A.pegawaicuti_status_permohonan = ref_status_permohonan_cuti.status_id');
        $this->db->join('ref_pangkat_golongan', 'P.pegawai_pangkat_terakhir_id = ref_pangkat_golongan.pangkat_golongan_id');
        $this->db->join('ref_unit U', 'P.pegawai_unit_id = U.unit_id', 'LEFT');
        $this->db->where('A.pegawaicuti_id', $id);
