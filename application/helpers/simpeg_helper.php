...
if (!function_exists('terbilang')) {
    ...
    function status_permohonan_cuti( $status = null, $alasan = null ){
        if( $status == '1' ){
            ...
            $status_text = "Ditolak";
            return "<strong>".$status_text."</strong><br>(".$alasan.")";
    ...
    {
        function sk_cuti($id=null,$file_sk=null, $proses=null, $menu=null)
        {
    ...
    if(!empty($file_sk)){
      ...
      return '<a href="#" class="btn '.$class.' btn-sm" type="button" onclick="view(\''.$id.'\',\''.$proses.'\',\''.$file_sk.'\')" ><i class="fa fa-file"></i>&nbsp;&nbsp; Lihat </a>';
    }else{
      return '';
    }
    }else{
    ...
    function send_wa($curl, $number, $message)
    {
    ...
    function send_media($curl, $number, $message, $file)
    {
        curl_setopt_array($curl, array(
    ...
    function xucwords($string)
    {
        ...
        {
            if(!preg_match("/\b(?=[LXIVCDM]+\b)([a-z]+)\b/i", $word)) {
                $word = ucfirst($word);
            } else {
    	...
    	$return = str_replace("Sd", "SD", $return);
    	$return = str_replace("Smp", "SMP", $return);
    	$return = str_replace("Tk", "TK", $return);
    	$return = str_replace("Sdlb", "SDLB", $return);
    	$return = str_replace("Upt", "UPT", $return);
    	$return = str_replace("Rsud", "RSUD", $return);
    	$return = str_replace("Dprd", "DPRD", $return);
    	$return = str_replace("&", "dan", $return);
    	
    	return $return;
    }
  
    function fixromawi($string)
    {
