...
class M_data_pengajuan_mutasi extends MY_Model {
  ...
  function json()
  {
    ...
    $this->datatables->select('A.usulan_id,A.usulan_nip,A.usulan_status,B.pegawai_nama,A.insert_time,keterangan,CASE WHEN usulan_jenis = "1" THEN "Keluar Kabupaten/Kota" ELSE "Antar Instansi" END AS usulan_jenis,"" AS action,file_sk', FALSE);
    ...
    $this->datatables->edit_column('file_sk', '$1', 'get_sk_mutasi(file_sk,usulan_status,"admin",usulan_id)');
    $this->datatables->edit_column('usulan_status', '$1', 'get_usulan_status(usulan_status)');
    $this->datatables->add_column('action', '$1', 'get_action_mutasi(usulan_id,usulan_status,"admin")');
